...
export async function getOperationalSummary(
  ...
  const branchWhere: Prisma.BranchWhereInput = branchIdFilter
    ? { id: branchIdFilter }
    : {};

  const [branches, categories, missions, tasks, merchants, leads, leadsForBanks, territoryCells, deploymentRows, dailyReports, activityByAction, activityByBranch] =
    await Promise.all([
...
async function getMerchantsByBranch(
  ...
  const userToBranch = new Map(users.map((u) => [u.id, u.branchId]).filter(([, b]) => b));
  const branchIds = [...new Set(userToBranch.values())] as string[];
  const branchMap = await getBranchNameMap(branchIds);
...
type LeadBankRow = { zoneBranchId: string | null; scoutedByBranchId: string | null; externalBankIds: string[] };
...
async function getActivityCountsByAction(
  ...
  const where: Prisma.ActivityLogWhereInput = branchId ? { branchId } : { branchId: { not: null } };
  if (from || to) {
    where.createdAt = {};
    if (from) where.createdAt.gte = from;
    if (to) where.createdAt.lte = new Date(to.toISOString().slice(0, 10) + "T23:59:59.999Z");
