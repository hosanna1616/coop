...
export async function getUsersForAdmin(
  ...
  { users: { id: string; name: string; role: string; teamName: string | null; branchName: string | null }[]; total: number }
> {
  ...
  try {
    if (session.role === "ADMIN") {
      const where: { role?: { not: string }; OR?: { branchId: string }[] | { team: { branchId: string } }[] } =
        branchIdFilter != null
          ? { role: { not: "ADMIN" }, OR: [{ branchId: branchIdFilter }, { team: { branchId: branchIdFilter } }] }
          : { role: { not: "ADMIN" } };
...
export async function getTeamsForAdmin(branchIdFilter?: string | null, options?: { limit?: number; offset?: number }): Promise<
  { teams: { id: string; name: string; branchId: string | null; branchName: string | null }[]; total: number }
> {
...
export async function getBranchesForAdmin(): Promise<
  { id: string; branchCode: string | null; companyName: string }[]
> {
  const { getBranchesFromDb } = await import("@/app/actions/branches");
  const branches = await getBranchesFromDb();
